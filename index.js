  ...
  getPixels(req.file.path, "image/png", function (err, pixels) {
    ...
    for (var i = 0; i < 2000000; i++) {
      ...
      if (VALID_COLORS.indexOf(hex) === -1) {
        res.send(
          `Pixel op ${i % 2000}, ${Math.floor(i / 2000)} heeft ongeldige kleur.`
        );
        fs.unlinkSync(req.file.path);
        return;
      }
    ...
    wsServer.clients.forEach((client) =>
      client.send(
        JSON.stringify({ type: "map", data: file, reason: req.body.reason })
      )
    );
...
wsServer.on("connection", (socket) => {
  ...
  socket.on("message", (message) => {
    ...
    try {
      ...
      socket.send(
        JSON.stringify({ type: "error", data: "Failed to parse message!" })
      );
      return;
    }
    ...
    if (!data.type) {
      socket.send(
        JSON.stringify({ type: "error", data: "Data missing type!" })
      );
    }
